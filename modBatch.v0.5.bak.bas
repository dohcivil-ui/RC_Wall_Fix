...
' Version: 0.5 (Step 3 + Phase A3 fc-fixed replicates for paper Section 5)
' Date: 2569 (2026)
'
...
Private Sub ShowSummaryStep3_A3(filePath As String, totalBatchTime As Double)
    ...
    ' Per (H, fc, algo) accumulators
    ' Track first cost seen + count of consistent + count of total + min/max iter
    Dim a3Count(1 To 3, 1 To 3, 1 To 2) As Integer
    Dim a3Valid(1 To 3, 1 To 3, 1 To 2) As Integer
    Dim a3FirstCost(1 To 3, 1 To 3, 1 To 2) As Double
    Dim a3Consistent(1 To 3, 1 To 3, 1 To 2) As Integer
    Dim a3MinIter(1 To 3, 1 To 3, 1 To 2) As Long
    Dim a3MaxIter(1 To 3, 1 To 3, 1 To 2) As Long
    Dim a3MinCost(1 To 3, 1 To 3, 1 To 2) As Double
    Dim a3MaxCost(1 To 3, 1 To 3, 1 To 2) As Double
    
    For i = 1 To 3
        For j = 1 To 3
            For k = 1 To 2
                a3MinCost(i, j, k) = 1E+15
                a3MinIter(i, j, k) = 999999999
                a3FirstCost(i, j, k) = -1
            Next k
        Next j
    Next i
    ...
    Do While Not EOF(fileNum)
        ...
        If UBound(parts) >= 28 Then
            ...
            a3Count(hIdx, fcIdx, algoIdx) = a3Count(hIdx, fcIdx, algoIdx) + 1
            
            If isValid = 1 Then
                a3Valid(hIdx, fcIdx, algoIdx) = a3Valid(hIdx, fcIdx, algoIdx) + 1
                
                ' Track first valid cost; count consistent matches
                If a3FirstCost(hIdx, fcIdx, algoIdx) < 0 Then
                    a3FirstCost(hIdx, fcIdx, algoIdx) = cost
                    a3Consistent(hIdx, fcIdx, algoIdx) = 1
                Else
                    If Abs(cost - a3FirstCost(hIdx, fcIdx, algoIdx)) < 0.01 Then
                        a3Consistent(hIdx, fcIdx, algoIdx) = a3Consistent(hIdx, fcIdx, algoIdx) + 1
                    End If
                End If
                
                If cost < a3MinCost(hIdx, fcIdx, algoIdx) Then a3MinCost(hIdx, fcIdx, algoIdx) = cost
                If cost > a3MaxCost(hIdx, fcIdx, algoIdx) Then a3MaxCost(hIdx, fcIdx, algoIdx) = cost
                If iterBest < a3MinIter(hIdx, fcIdx, algoIdx) Then a3MinIter(hIdx, fcIdx, algoIdx) = iterBest
                If iterBest > a3MaxIter(hIdx, fcIdx, algoIdx) Then a3MaxIter(hIdx, fcIdx, algoIdx) = iterBest
            End If
        End If
NextA3:
    Loop
    ...
    msg = "Phase A3 done!" & vbCrLf & _
          ...
          "Format: H/fc -- BA cost(consist/valid) vs HCA cost(consist/valid)" & vbCrLf & vbCrLf
    ...
    For i = 1 To 3   ' H
        For j = 1 To 3   ' fc
            baCost = a3FirstCost(i, j, 1)
            hcaCost = a3FirstCost(i, j, 2)
            baIter = a3MinIter(i, j, 1)
            hcaIter = a3MinIter(i, j, 2)
            
            ' Verdict: BA <= HCA on both cost and iter?
            If baCost <= hcaCost + 0.01 And baIter <= hcaIter Then
                verdict = "BA wins"
            ElseIf baCost > hcaCost + 0.01 Then
                verdict = "HCA cost lower"
            ElseIf baIter > hcaIter Then
                verdict = "HCA iter lower"
            Else
                verdict = "tie"
            End If
            
            msg = msg & "H=" & hVals(i) & " fc=" & fcVals(j) & ":  " & _
                  "BA " & Format(baCost, "#,##0") & "(" & a3Consistent(i, j, 1) & "/" & a3Valid(i, j, 1) & "," & baIter & "it)  " & _
                  "HCA " & Format(hcaCost, "#,##0") & "(" & a3Consistent(i, j, 2) & "/" & a3Valid(i, j, 2) & "," & hcaIter & "it)  " & _
                  "[" & verdict & "]" & vbCrLf
        Next j
    Next i
