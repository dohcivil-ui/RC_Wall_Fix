...
Public Sub BeginSearch(budget As Long, seed As Long, algorithm As String, Optional trial As Long = 1)
    ...
    Loop
    MkDir RunFolder
    ReDim EvaluationCSV(0 To budget)
    EvaluationCSV(0) = "evaluation,entry,valid,reason,cost,best_cost,tt,tb,TBase,Base,LToe,StemDB,StemSP,ToeDB,ToeSP,HeelDB,HeelSP" & vbCrLf
End Sub
...
Public Function EvaluateCandidate(d As Design, entry As String, ByRef candidateCost As Double) As Boolean
    ...
    EvaluationCSV(EvaluationCount) = EvaluationCount & "," & entry & "," & CStr(ok) & "," & Replace(LastValidationReason, ",", ";") & "," & _
