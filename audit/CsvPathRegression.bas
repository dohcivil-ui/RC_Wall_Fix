...
Private Sub CheckMethod(method As String)
    ...
    Verify method & " completed short trial", EvaluationCount = 8 And RunTrial = 1
    ...
    stem = "loopPrice-" & method & "-H5"
    firstPath = UniqueExportPath(stem)
    If method = "BA" Then SaveLoopPriceCSV_BA 5 Else SaveLoopPriceCSV 5
    Verify method & " loop CSV saved", Len(Dir$(firstPath)) > 0
    Verify method & " repeated loop filename is unique", UniqueExportPath(stem) <> firstPath
    If method = "BA" Then SaveLoopPriceCSV_BA 5 Else SaveLoopPriceCSV 5
    Verify method & " second loop CSV preserves first", Len(Dir$(firstPath)) > 0 And Len(Dir$(RunFolder & "\" & stem & "-1.csv")) > 0
...
Public Sub Main()
    ...
    Form1.txtMaxIter.Text = "8": Form1.txtTrials.Text = "1": Form1.txtSeed.Text = "20260909"
