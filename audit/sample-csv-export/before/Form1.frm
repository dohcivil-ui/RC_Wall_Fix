...
Private Sub cmdBA_Click()
    ...
    Call BeginTrialAcceptExport(CLng(numTrials))
    ...
    For trial = 1 To numTrials
        ...
        Call AppendTrialAcceptExport
        
        Debug.Print "Trial " & trial & ": Cost=" & Format(trialCost, "#,##0.00")
        
        If trialDesign.IsValid And ((trialCost < globalBestCost) Or _
           (trialCost = globalBestCost And modDataStructures.BestCostIteration > 0 And _
            (globalBestIteration = 0 Or modDataStructures.BestCostIteration < globalBestIteration))) Then
    ...
    AddResultLine "accept CSV (all trials): " & LastTrialAcceptCSVPath
...
Private Sub ShowRunExportError(ByVal message As String)
    ...
    If Len(LastTrialAcceptCSVPath) > 0 Then
        AddResultLine "Saved trial data (may be incomplete): " & LastTrialAcceptCSVPath
    End If
End Sub
    ...
    For trial = 1 To numTrials
       ...
       ' เปรียบเทียบกับ Global Best
        If bestDesign.IsValid And ((currentCost < globalBestCost) Or _
           (currentCost = globalBestCost And modDataStructures.BestCostIteration > 0 And _
            (globalBestIteration = 0 Or modDataStructures.BestCostIteration < globalBestIteration))) Then
...
Private Function ValidateInputs() As Boolean
    ...
    If CDbl(txtH1.Text) > CDbl(txtH.Text) Then Exit Function
