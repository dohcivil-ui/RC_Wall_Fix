...
Public Const RESULT_CSV_ROOT As String = "C:\reserch 69\RC_Wall_Fix\audit\ten-trials-h3\geometry-hold-replication-10\output"
...
Public Sub BeginSearch(budget As Long, algorithm As String, Optional trial As Long = 1)
    Dim emptyDesign As Design
    If budget < 1 Then Err.Raise 5, , "Evaluation budget must be positive"
    Call InitializeSearchRandom
    EvaluationCount = 0: EvaluationBudget = budget
...
Public Function EvaluateCandidate(d As Design, entry As String, ByRef candidateCost As Double) As Boolean
    ...
    TenTrialProbe.TraceCandidate d, entry, candidateCost
...
Public Sub InitializeSearchRandom()
    ...
    ' Initialize once from the system timer; continue the stream across trials and clicks.
