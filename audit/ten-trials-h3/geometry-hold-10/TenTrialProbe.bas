...
Public Sub RecordTrial()
    ...
    Print #f, RunAlgorithm & "," & RunTrial & "," & EvaluationCount & "," & CsvNumber(RunBestCost) & "," & RunBestEvaluation & "," & CStr(RunBest.IsValid) & "," & CsvNumber(RunBest.FS_OT) & "," & CsvNumber(RunBest.FS_SL) & "," & CsvNumber(RunBest.FS_BC) & "," & RunRecoveryCount & "," & CsvNumber(RunBest.tb) & "," & CsvNumber(RunBest.TBase) & "," & CsvNumber(RunBest.Base)
    Close #f
...
Private Sub RunMethod(method As String)
    ...
    Close #traceFile
    traceFile = 0
    If RunTrial <> 10 Or RunFolder <> "" Then failures = failures + 1
    f = FreeFile
...
Public Sub Main()
    ...
    Print #f, "algorithm,trial,evaluations,best_cost,best_evaluation,valid,FS_OT,FS_SL,FS_BC,recoveries,tb,TBase,Base"
    Close #f
    Load Form1
    Form1.txtH.Text = "3": Form1.txtH1.Text = "1.2"
    ...
    Form1.txtMaxIter.Text = "5000": Form1.txtTrials.Text = "10"
    Form1.cboConcreteStrength.ListIndex = 2
    RunMethod "BA"
    RunMethod "HCA"
    If trialCount <> 20 Then failures = failures + 1
