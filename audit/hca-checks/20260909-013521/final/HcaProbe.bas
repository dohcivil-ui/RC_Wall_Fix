...
Public Sub Main()
    ...
    trial = HillClimbingOptimization(125, 5, 1.2, 1.8, 2.4, 30, 0.6, 30, 0.075, mat, True, 1, 24, 40, 63, 87, 101, 110, 100, 110, 101, 110, 12365)
    Print #f, "HCA_LOCAL125 cost=" & RunBestCost & "; refinements=" & HCARefinementCount & "; folder=" & RunFolder
    If RunBestCost >= 8573.88408 Then Print #f, "FAIL: missed cheaper local steel pair"
    trial = HillClimbingOptimization(5000, 5, 1.2, 1.8, 2.4, 30, 0.6, 30, 0.075, mat, RandomSeed:=12345)
    Print #f, "HCA5000 seed12345 cost=" & RunBestCost & "; folder=" & RunFolder
    trial = HillClimbingOptimization(5000, 5, 1.2, 1.8, 2.4, 30, 0.6, 30, 0.075, mat, RandomSeed:=12374)
    Print #f, "HCA5000 seed12374 cost=" & RunBestCost & "; folder=" & RunFolder
    trial = HillClimbingOptimization(125, 5, 1.2, 1.8, 2.4, 30, 0.6, 0.01, 0.075, mat, RandomSeed:=12345)
    Print #f, "HCA_NO_SOLUTION valid=" & RunBest.IsValid & "; folder=" & RunFolder
    If RunBest.IsValid Then Print #f, "FAIL: no-solution fixture"
    trial = BisectionOptimization(5000, 5, 1.2, 1.8, 2.4, 30, 0.6, 30, 0.075, mat, RandomSeed:=12345)
    Print #f, "BA5000 seed12345 cost=" & RunBestCost & "; folder=" & RunFolder
    mat = GetSD40Material(240, GetConcretePrice(240), 24)
    trial = HillClimbingOptimization(125, 3, 1.2, 1.8, 2.4, 30, 0.6, 30, 0.075, mat, RandomSeed:=12345)
    Print #f, "HCA_H3 cost=" & RunBestCost & "; folder=" & RunFolder
    mat = GetSD40Material(280, GetConcretePrice(280), 24)
    trial = HillClimbingOptimization(125, 4, 1.2, 1.8, 2.4, 30, 0.6, 30, 0.075, mat, RandomSeed:=12345)
    Print #f, "HCA_H4 cost=" & RunBestCost & "; folder=" & RunFolder
    Close #f
    Exit Sub
Failed:
