"""Independent checks of the authorized Es/Ec material model in native VB6.
...
for fc_prime, fy in [(180,3000),(210,4000),(240,4000),(280,4000),(320,4000)]:
    ...
    R = fc*k*j/2
    rho = fc*k/(2*fs)
    rows.append(dict(fc_prime=fc_prime,fy=fy,Es=2040000,Ec_reference=ec,n_reference=n_reference,Ec_used=ec_model,n=n,k=k,j=j,R=R,rho_bal=rho))
    vb.append(f'    refParams = CalculateWSDParameters({fy}, {fc_prime})')
    for field,value in [('n',n),('k',k),('j',j),('R',R)]:
        vb.append(f'    Call Near("reference Es/Ec fc{fc_prime} {field}", refParams.{field}, {value:.12f})')
    vb.append(f'    Call Near("reference Es/Ec fc{fc_prime} balanced rho", CalculateRhoBalanced(refParams), {rho:.12f})')
    ...
    for field,value in [('ca',fc),('sa',fs),('ja',j)]:
        vb.append(f'    Call Near("reference Es/Ec fc{fc_prime} actual balanced {field}", {field}, {value:.12f})')
...
(P/'wsd-reference-independent.json').write_text(json.dumps(dict(materials=rows,actual_DB16=actual,
    pdf_example_note='The PDF example rounds n for fc_prime=210; current project uses unrounded Es/Ec after checking decision sensitivity.',
    model_note='User authorized Es/Ec when fixed n changes acceptance decisions. Use unrounded Es/Ec; complete EIT compliance remains unverified.'),indent=2),encoding='ascii')
